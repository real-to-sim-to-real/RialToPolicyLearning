...
class TD3(object):
    ...
    def train(self, replay_buffer, iterations, batch_size=100, discount=0.99, tau=0.005, policy_noise=0.2, noise_clip=0.5, policy_freq=2, policy_eval=False):
        import tqdm
    ...
    def actor_bc_train(self, expert_replay_buffer, validation_buffer, iterations, batch_size=100):
        import tqdm
        print(self.actor)
        print(len(expert_replay_buffer.storage))
        with tqdm.trange(iterations, leave=False) as ranger:
            for it in ranger:

                # Sample replay buffer 
                x, g, u = expert_replay_buffer.sample(batch_size)
                state = torch.FloatTensor(x).to(device)
                goal = torch.FloatTensor(g).to(device)
                action = torch.FloatTensor(u).to(device)
                actor_loss = F.mse_loss(self.actor(state, goal), action)
                # print(self.actor(state, goal), action)
                self.actor_optimizer.zero_grad()
                actor_loss.backward()
                self.actor_optimizer.step()
                x, g, u = validation_buffer.sample(batch_size)
                state = torch.FloatTensor(x).to(device)
                action = torch.FloatTensor(u).to(device)
                goal = torch.FloatTensor(g).to(device)
                validation_loss = F.mse_loss(self.actor(state, goal), action)
                ranger.set_description('Actor loss: %.03f Validation Loss: %.03f'% (ptu.to_numpy(actor_loss), ptu.to_numpy(validation_loss)))
