...
def prepare_for_pointnet(coords):
    lens = []
    unpadded_points = []
    for points in coords:
        points = unpad_points(points)
        lens.append(len(points))
        unpadded_points.append(points)
    
    final_points = []
    for point in unpadded_points:
        indices = np.random.choice(coord.shape[0], int(num_points), replace=False)
        final_points.append(point[indices])
