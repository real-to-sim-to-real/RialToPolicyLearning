...
class DifferentialInverseKinematics:
    ...
    def __init__(self, cfg: DifferentialInverseKinematicsCfg, num_robots: int, device: str):
        ...
        self._device = device
        # check valid input
        if self.cfg.ik_method not in ["pinv", "lstsq", "svd", "trans", "dls"]:
    ...
    def compute(
        ...
        # from huge.franka.diff_ik_utils import diffik_step
        # from utils_frames import th_quat_from_isaac_to_real
        # current_ee_rot_real = th_quat_from_isaac_to_real(current_ee_rot)
        # desired_ee_rot_real = th_quat_from_isaac_to_real(self.desired_ee_rot)
        # # TODO: correct for the difference in convention
        
        # delta_joint_positions = diffik_step(current_ee_pos, current_ee_rot_real, self.desired_ee_pos, desired_ee_rot_real, jacobian)
        
        position_error, axis_angle_error = compute_pose_error(
            current_ee_pos, current_ee_rot, self.desired_ee_pos, self.desired_ee_rot, rot_error_type="axis_angle"
        )
    ...
    def _compute_delta_dof_pos(self, delta_pose: torch.Tensor, jacobian: torch.Tensor) -> torch.Tensor:
        ...
        elif self.cfg.ik_method == "lstsq":
            dt = self._ik_params["dt"]
            delta_dof_pos = torch.linalg.lstsq(jacobian, delta_pose.unsqueeze(-1)).solution  
            delta_dof_pos = dt*delta_dof_pos
            delta_dof_pos = delta_dof_pos.squeeze(-1)
