import numpy as np
from rialto.envs import goal_env
import gym
...
class GymGoalEnvWrapper(goal_env.GoalEnv):
    ...
    def _base_obs_to_state(self, base_obs):
        obs = base_obs[self.obs_key]
        goal = base_obs[self.goal_key]
        sgoal = base_obs[self.sgoal_key]
        return np.concatenate([obs, goal, sgoal], axis=1)
    ...
    def sample_goal(self):
        ...
        goal = base_obs[desired_key][0].flatten()
        sgoal = base_obs[desired_state_key][0].flatten()
